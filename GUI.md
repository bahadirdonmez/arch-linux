...
1. Thunar Archive Plugin allows you to create and extract archive files using context
menu items.
...
2. p7zip is a command-line port of 7-Zip for POSIX systems, including Linux:

    ```bash
    sudo pacman -S p7zip
    ```

3. File Roller is an archive manager for the GNOME desktop environment:

    ```bash
    sudo pacman -S file-roller
    ```

    Thunar Archive Plugin acts as a frontend for File Roller.

### Screenshots

Flameshot is a Qt5-based software for interactive screenshot taking. Select the desired
area, draw with various tools, and enjoy the customization capabilities.
...
3. Open the Flameshot configuration menu:
    ...
    - Check the contents of the `~/.config/flameshot/flameshot.ini` file and correct
    any errors:
...
4. Go to **Settings > Keyboard > Application Shortcuts**:
    ...
    - Click **Add** and type `flameshot full -c -p ~/Pictures/Screenshots`.
...
```bash
sudo mkdir -p /usr/local/share/fonts/otf /usr/local/share/fonts/ttf 
...
Install Google's Noto font family with full Unicode coverage, including its emoji and
CJK optional dependencies:
...
3. Configure HiDPI settings for your desktop environment:
...
1. To adjust the GRUB font size, convert the `DejaVuSansMono.ttf` font to a format that
GRUB can utilize:
...
2. Modify the `/etc/default/grub` file to set the new font:
...
3. Check the contents of the `/etc/default/grub` file and correct any errors:
...
4. Regenerate the GRUB configuration file:
...
1. Change to the `git` directory and clone the `Qogir-theme` repository by running the
following command:
...
2. Change to the `Qogir-theme` directory and run the installation script by executing
...
1. Change to the `git` directory and clone the `Qogir-icon-theme` repository by running
the following command:
...
2. Change to the `Qogir-icon-theme` directory and execute the installation script by
...
1. Create a directory to store single-monitor wallpapers and copy them to this new
...
2. Create a directory to store dual-monitor wallpapers and copy them to this new
directory by running the following command:
...
1. Configure Plank Dock according to your needs:
    ...
    - Open **Firefox**, **File Manager**, **Terminal**, **VS Code**, and **Flameshot**
    applications, right-click on the icons in the dock, and check the box labeled
    **Keep in Dock** to pin them to the dock.
...
Mugshot is a lightweight user configuration utility for Linux, allowing you to quickly
update your personal profile and synchronize the changes across applications.
...
1. Move the user avatar to `/usr/share/pixmaps/users` by entering the following command:

    ```bash
    cp -r ~/"Google Drive"/Personal/"Profile Pictures"/bahadir_profile_1_1_480px.png ~/Downloads
    sudo sh -c 'mkdir -p /usr/share/pixmaps/users && mv /home/Bahadir/Downloads/bahadir_profile_1_1_480px.png /usr/share/pixmaps/users/Bahadir.png'
    ```

2. Launch Mugshot from the menu or with the following command to configure it for your
needs:
...
**Properties**:
    ...
    - In the **Appearance** tab:
        ...
        - Check **Position profile on bottom**.
    ...
    - In the **Commands** tab:
        ...
        - Deselect all options except for **Settings Manager** and **Lock Screen**.
...
6. To configure the **Whisker Menu**:
    ...
    **Remove from favorites**.
    ...
    - Locate the **Firefox**, **File Manager**, **Terminal**, **VS Code**, and
    **Flameshot** applications. Right-click on each of them and select
    **Add to favorites**.
...
To customize the appearance of Firefox, follow these steps:

1. Navigate to **Settings > General**:

2. Under **Language and Appearance**:

    - Set **Default font** to `SF Pro`.
...
1. Go to **Settings > Panel > Panel 1**:
    ...
    - Under **Items**:
        ...
        - Remove **Notification Plugin** from the list.

        - Click on **Clock** options:

            - Set **Appearance: Layout** to **Digital**.

            - Set **Tooltip format** to **Custom Format**: `%H:%M%n%A, %d %B %Y`.
