...
2. Go to the [Arch Linux Downloads](https://www.archlinux.org/download/) page.
    ...
    - Download the `.iso` file and the respective `.iso.sig` file, for instance:
        ...
        - `archlinux-2023.03.01-x86_64.iso.sig`

    - Alternatively, you can use the `curl` command to download these files:

        ```shell
        $ curl -O https://geo.mirror.pkgbuild.com/iso/2023.03.01/archlinux-2023.03.01-x86_64.iso
        ```

        ```shell
        $ curl -O https://geo.mirror.pkgbuild.com/iso/2023.03.01/archlinux-2023.03.01-x86_64.iso.sig
        ```
...
image:
    ...
    ```shell
    gpg: die unterzeichneten Daten sind wohl in 'archlinux-2023.03.01-x86_64.iso'
...
2. Use the following command to write the installation image to the USB drive:
    ...
    ```shell
    $ sudo dd bs=4M if=archlinux-2023.03.01-x86_64.iso of=/dev/sdb conv=fsync oflag=direct status=progress
    ```
